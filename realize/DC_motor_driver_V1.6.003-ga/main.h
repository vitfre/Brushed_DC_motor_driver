...
//---------------------------------------------------------------------------------------
#define START_WORK_2	FLAGS.SETT_WORK_2=0x01;	 (PORTA|=(1<<3));	FLAGS.HOLD_M_4=0x02;	(PORTD&=~(1<<7));
...
void func_reg_power(void)
{
	...
	if ((time_diff!=0x00)&&(FLAGS.SETT_WORK_1!=0x00))
	{
		...
		{
			//---------------------------------------------------------------------------------------
			power=min_power+set_speed/2.24;
			//---------------------------------------------------------------------------------------
		} 
		else
		{
			//---------------------------------------------------------------------------------------
			FLAGS.POWER_SETT=0x01;
			if ((set_speed-3)<RPS)
			{
				//---------------------------------------------------------------------------------------
				if (power_dif>1)
			...
			//---------------------------------------------------------------------------------------
			power=min_power+set_speed/4+power_dif;
			//power=min_power+set_speed/8+power_dif;
			//power=min_power+set_speed/16+power_dif;
			//---------------------------------------------------------------------------------------
		};
		//---------------------------------------------------------------------------------------
		/*
	...
	switch (BUT_GetKey())		// Читаємо буфер клавіатури і переходим на відповідний case в залежності від коду кнопки
	{
		...
		case 0x02 :
		{
			...
			if (FLAGS.SETT_WORK_2==0x01)
			{
				...
				beep_tone(10);
				STOP_WORK_2
				eeprom_write_word((uint16_t*)0x10,(uint16_t)vork_time);
				//---------------------------------------------------------------------------------------
			}
			else
			{
